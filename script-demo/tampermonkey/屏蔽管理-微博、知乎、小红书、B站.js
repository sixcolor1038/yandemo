...
(function() {
    ...
    const siteConfigs = {
        zhihu: {
            ...
            likeCountSelector: '.VoteButton--up .Button-zi',
            // 新增文章和盐选相关选择器
...
function createManagementUI() {
    ...
    const tabHtml = `
        <div class="kb-tab-container">
            ...
            ${currentSite === 'zhihu' ? '<button class="kb-tab" data-tab="low-like">低赞屏蔽</button>' : ''}
    ...
    const contentHtml = `
        ...
        <!-- 知乎低赞屏蔽标签页 -->
        ${currentSite === 'zhihu' ? `
        <div id="kb-tab-low-like" class="kb-tab-content">
            <div class="kb-low-like-section">
                <div class="kb-low-like-title">
                    <span class="kb-status-indicator ${ZHIHU_LOW_LIKE_SETTINGS.enabled ? 'kb-status-enabled' : 'kb-status-disabled'}"></span>
                    知乎低赞内容屏蔽
                </div>
                <div class="kb-low-like-desc">
...
function parseLikeCount(likeText) {
    ...
    if (likeText.includes('万')) {
        const num = parseFloat(likeText);
        return isNaN(num) ? null : Math.round(num * 10000);
    }

    // 处理普通数字
    const num = parseInt(likeText.replace(/,/g, ''));
...
function updateZhihuLowLikeTabUI() {
    ...
    const statusIndicator = document.querySelector('#kb-tab-low-like .kb-status-indicator');
